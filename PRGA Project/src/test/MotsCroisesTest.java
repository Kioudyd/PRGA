...
import tp2.motCroiseAvecHeritage;
...
{
	private motCroiseAvecHeritage mc ;
	...
	public void setUp() throws Exception
	{
		hauteur = 321 ;
		largeur = 654 ;
		mc = new motCroiseAvecHeritage(hauteur, largeur) ;
		...
		for (int lig=1; lig <=mc.getheight(); lig++ )
		{
			for (int col=1; col <=mc.getWidth(); col++ )
			{
		...
		motCroiseAvecHeritage mc2 = new motCroiseAvecHeritage(2, 4);
		for (int lig=1; lig <=mc2.getheight(); lig++ )
		{
			for (int col=1; col <=mc2.getWidth(); col++ )
			{
	...
	public void testCasesNoires()
	{
		for (int lig=1; lig<=mc.getheight(); lig++)
			for (int col=1; col<=mc.getWidth(); col++)
			{
				...
				{
					assertTrue("La case (" + lig + "," + col + ") doit être noire",
									mc.estCaseNoire(lig, col)) ;
				}
				else
				{
					assertFalse("La case (" + lig + "," + col + ") ne doit pas être noire",
									mc.estCaseNoire(lig, col)) ;
				}
			}
	}
